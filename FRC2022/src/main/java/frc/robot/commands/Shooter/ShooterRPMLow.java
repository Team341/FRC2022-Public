...
public class ShooterRPMLow extends CommandBase {
   ...
   */
  public ShooterRPMLow(Shooter shooter, Tower tower, boolean enableLimelight,
      DoubleSupplier defaultRPM, boolean end) {
    // Use addRequirements() here to declare subsystem dependencies.
    mShooter = shooter;
    mPhotonVisionInterface = PhotonVisionInterface.getInstance();
    rpm = Constants.Shooter.DEFAULT_LOW_RPM;
    calculatedSpeed = 0.0;
    // rpmThreshold = 50.0;
    mIsManual = false;
  ...
  public void initialize() {
    ...
    if (mIsManual) {
      ...
      rpm = mShooter.getRPMFromTableLow(mPhotonVisionInterface.getDistance());
    }
    if (mEnableLimelight) {
  ...
  public boolean isFinished() {
    // return mTower.getTopBallState() == BallState.Empty &&
    // mTower.getBottomBallState() == BallState.Empty;
    return count > 30 && end;
  }

  private void logToDashboard() {
    SmartDashboard.putNumber("Shooter/Shoot Count", count);
